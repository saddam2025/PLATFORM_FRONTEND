          ...
          className={
            isActive
              ? `relative flex items-center gap-4 justify-start flex-row-reverse rounded-l-full
                 bg-surface-canvas text-sidebar font-bold px-6 py-3 transition-transform duration-200
              ...
              : `flex items-center gap-4 justify-start flex-row-reverse text-white/60 hover:text-white
                 hover:bg-sidebar-soft rounded-xl px-6 py-3 transition-all duration-300`
          }
        >
          <span
...
export default function Sidebar() {
      ...
      <div className="flex flex-col gap-2">
        {instructors.slice(0, 4).map((ins) => (
          <NavLink
            ...
            className="flex items-center gap-3 px-2 py-2 rounded-lg text-white/80 hover:bg-sidebar-soft hover:text-white transition-colors duration-300"
          >
  ...
  const ChangeInstructorButton = () => (
    <Button
      variant="ghost"
      size="sm"
      className="w-full !bg-white/10 !text-white !border-white/20 hover:!bg-white/20 hover:scale-[1.02] active:scale-95 transition-all duration-300"
    >
      تغيير المدرس
    </Button>
  );
  ...
  const LogoutLink = () => (
    <button
      type="button"
      onClick={logout}
      className="group mt-6 flex items-center gap-4 justify-start flex-row-reverse text-white/60 hover:text-white transition-colors duration-300 px-6 py-3"
    >
  ...
  if (role === 'assistant') {
    return (
      <div className="flex flex-col h-full bg-sidebar text-white px-4 py-8 overflow-y-auto">
