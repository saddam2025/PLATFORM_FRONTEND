...
export default function ScratchCardManager() {
  ...
  const copyCodes = async (codes) => {
    try {
      ...
      setError('');
    } catch { setError('تعذر نسخ الأكواد.'); }
  };

  const copyCode = async (code) => { try { await navigator.clipboard.writeText(code); setCopiedCode(code); setTimeout(() => setCopiedCode(''), 1500); } catch { setError('تعذر نسخ الكود.'); } };
  ...
  return <div dir="rtl" className="space-y-6"><CourseAccessCodeGenerator instructorId={instructorId} canGenerate={canGenerate} /><AccessCodeBatchList instructorId={instructorId} /><div><h1 className="text-xl font-semibold text-ink-900">أكواد التفعيل</h1><p className="text-sm text-ink-500 mt-1">الأكواد تظهر مرة واحدة عند التوليد فقط.</p></div>{error && <div className="rounded-md p-3 bg-danger-soft text-danger-DEFAULT">{error}</div>}{!canGenerate && <Badge variant="danger">لا تملك صلاحية توليد الأكواد.</Badge>}
    ...
    {activeTab === 'scratchcard' && <div className="space-y-6"><form onSubmit={handleGenerateScratchCards} className="bg-surface-default rounded-2xl shadow-card p-6 grid grid-cols-1 md:grid-cols-4 gap-4 items-end"><div><label className="block text-sm mb-1">عدد البطاقات</label><Input type="number" min={1} value={count} onChange={(e) => setCount(e.target.value)} disabled={!canGenerate} /></div><div><label className="block text-sm mb-1">معرف الدفعة</label><Input value={batchId} onChange={(e) => setBatchId(e.target.value)} disabled={!canGenerate} /></div><div><label className="block text-sm mb-1">القيمة</label><Input type="number" min={1} value={value} onChange={(e) => setValue(e.target.value)} disabled={!canGenerate} /></div><Button type="submit" variant="primary" disabled={!canGenerate}>توليد</Button></form>
      {oneTimeCodes.length > 0 && <section className="bg-success-soft rounded-2xl p-6"><div className="mb-3 flex items-center justify-between gap-3"><h2 className="font-semibold">أكواد شحن المحفظة الجديدة — انسخها الآن، لن يمكن عرضها لاحقاً</h2><Button size="sm" variant="subtle" onClick={() => copyCodes(oneTimeCodes)}>نسخ كل الأكواد</Button></div><div className="grid grid-cols-2 md:grid-cols-4 gap-2">{oneTimeCodes.map((code) => <div key={code} className="flex items-center justify-between rounded bg-surface-default p-2"><code>{code}</code><Button type="button" size="sm" variant="ghost" onClick={() => copyCode(code)} aria-label="نسخ الكود">{copiedCode === code ? '✓' : 'نسخ'}</Button></div>)}</div></section>}
      <section className="bg-surface-default rounded-2xl shadow-card p-6 space-y-4"><div className="grid grid-cols-1 md:grid-cols-3 gap-3"><Input placeholder="معرف الدفعة" value={filterBatch} onChange={(e) => setFilterBatch(e.target.value)} /><select className="input" value={status} onChange={(e) => setStatus(e.target.value)}><option value="">كل الحالات</option><option value="available">متاح</option><option value="redeemed">مستخدم</option></select><Button variant="ghost" onClick={() => loadCards().catch((err) => setError(err?.message || 'تعذر التصفية.'))}>تطبيق الفلتر</Button></div><div className="text-xs text-ink-500">ملخص الدفعات (شحن محفظة): {batches.map((batch) => `${batch._id}: ${batch.redeemed}/${batch.total}`).join(' | ') || 'لا توجد دفعات'}</div><div className="overflow-x-auto"><table className="w-full text-sm text-right"><thead><tr className="border-b"><th className="p-2">النوع</th><th>معرف البطاقة</th><th>القيمة</th><th>الحالة</th><th>المستخدم</th><th>تاريخ الاستخدام</th><th>الدفعة</th></tr></thead><tbody>{cards.map((card) => <tr key={card._id} className="border-b"><td className="p-2"><Badge variant="info">شحن محفظة</Badge></td><td className="font-mono">{card._id}</td><td>{card.value} ج.م</td><td><Badge variant={card.isRedeemed ? 'success' : 'neutral'}>{card.isRedeemed ? 'مستخدم' : 'متاح'}</Badge></td><td>{card.redeemedBy?.name || '—'}</td><td>{card.redeemedAt ? new Date(card.redeemedAt).toLocaleDateString('ar') : '—'}</td><td>{card.batchId}</td></tr>)}{cards.length === 0 && <tr><td colSpan={7} className="p-6 text-center text-ink-500">لا توجد بطاقات مطابقة</td></tr>}</tbody></table></div></section></div>}
    {activeTab === 'lectureCodes' && <div className="space-y-4"><form onSubmit={handleGenerateLectureCodes} className="bg-surface-default rounded-2xl shadow-card p-6 grid grid-cols-1 md:grid-cols-3 gap-4 items-end"><div><label className="block text-sm mb-1">عدد الأكواد</label><Input type="number" min={1} value={lectureCount} onChange={(e) => setLectureCount(e.target.value)} disabled={!canGenerate} /></div><div><label className="block text-sm mb-1">الدورة / المحاضرة</label><select className="input w-full" value={courseId} onChange={(e) => setCourseId(e.target.value)} disabled={!canGenerate}>{courses.map((course) => <option key={course._id} value={course._id}>{course.title_ar || course.title_en}</option>)}</select></div><Button type="submit" variant="primary" disabled={!canGenerate || !courseId}>توليد</Button></form>{lectureCodes.length > 0 && <div className="bg-success-soft rounded-2xl p-6"><div className="mb-3 flex justify-end"><Button size="sm" variant="subtle" onClick={() => copyCodes(lectureCodes)}>نسخ كل الأكواد</Button></div><div className="grid grid-cols-2 md:grid-cols-4 gap-2">{lectureCodes.map((code) => <div key={code} className="flex items-center justify-between rounded bg-surface-default p-2"><code>{code}</code><Button type="button" size="sm" variant="ghost" onClick={() => copyCode(code)} aria-label="نسخ الكود">{copiedCode === code ? '✓' : 'نسخ'}</Button></div>)}</div></div>}</div>}
  </div>;
}
